...
| 2 | No UDS ISR blocking | **PASS** | `library/compat/legacy_diagnostics/uds_stm32/uds_stm32.c`, `App/Src/canopen_reference_uds.c`, strict fake-HAL and source review | ISR work is frame copy/queue/counter handling; UDS dispatch, Flash, and reset action are mainline-owned. Keep this as a source-contract pass, not a timing pass. |
...
| 6 | Correct ISO-TP state machine | **PARTIAL** | `library/compat/legacy_diagnostics/isotp`, `tests/isotp/test_isotp_contract.c`, adversarial tests | SF/FF/CF/FC, BS, STmin, sequence, timeout, overflow, and bounded FC WAIT are covered. The implementation is a deliberately narrow classic-CAN subset and lacks full ISO 15765-2 conformance evidence for every network-layer option. |
