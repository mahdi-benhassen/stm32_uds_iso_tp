...
This document defines the software ordering for UDS ECUReset on the STM32C092 FDCAN Classical CAN profile. ECUReset arms a pending flag; the UDS service and CAN path do not execute the MCU reset. The application mainline performs the platform reset handoff. The C092 example arms a platform-owned pending reset and polls it from the main loop; the current handoff wait is 50 ms. This value is not P2 timing and is not a measured reset-to-diagnostic-ready interval.
